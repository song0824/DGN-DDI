...
class SSI_DDI_Block(nn.Module):
    def __init__(self, n_heads, in_features, head_out_feats, final_out_feats):
        ...
        # 官方实现用 SAGPooling(min_score=-1)。当前环境 torch_scatter 无 CUDA/cp313
        # 轮子，改为图内 softmax 门控后再 global_add_pool，语义接近“保留全部节点”。
        self.pool_score = nn.Linear(n_heads * head_out_feats, 1)

    def forward(self, data):
        data.x = self.conv(data.x, data.edge_index)
        score = pyg_softmax(self.pool_score(data.x).squeeze(-1), data.batch)
        global_graph_emb = global_add_pool(data.x * score.unsqueeze(-1), data.batch)
